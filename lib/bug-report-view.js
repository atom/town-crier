...
export default class BugReportView {
  constructor (report) {
    ...
    this.disposable = atom.commands.add('.town-crier.bug-report', {
      'core:focus-next': () => {
        this.focusNext()
      },
      'core:focus-previous': () => {
  ...
  render () {
    return (
      <div className='town-crier bug-report pane-item native-key-bindings' tabIndex='-1'>
        ...
        <p>Please enter the steps you follow to reproduce the problem:</p>
        <div className='block'>
          <div className='block'>
            {this.renderReproSteps()}
            <TooltipView title='Add another step to reproduce' placement='auto'>
              <button className='btn' on={{click: this.didClickAddStep}} ref='tooltipTarget'>+</button>
            </TooltipView>
          </div>
          <div className='block'>
        ...
        <div className='block'>
          ...
          <div className={this.report.spinnerClass} id='spinner'>
            <span class='loading loading-spinner-tiny inline-block'></span>
            <span>{this.report.spinnerText}</span>
          </div>
        </div>
      </div>
    )
  ...
  updateModel () {
    ...
    this.report.additionalInformation.text = this.refs.additionalInformationInput.value
