...
${blocks}
  ...
  async submit () {
    try {
      ...
      if (!token) {
        atom.notifications.addWarning('Please sign in', {
          detail: 'You must sign in first using the GitHub package before you can submit issues.',
          dismissable: true,
          buttons: [
            {
              text: 'Open GitHub dock',
              onDidClick: () => {
                const view = atom.views.getView(atom.workspace)

                atom.commands.dispatch(view, 'github:toggle-github-tab-focus')
              }
            }
          ]
        })
