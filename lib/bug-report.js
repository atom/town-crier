...
export default class BugReport {
  constructor (data = {}) {
    ...
    this.additionalInformation = data.additionalInformation || {text: '', packageList: this.packageList}
    ...
    return {
      ...
      data: {
        ...
        additionalInformation: {
          text: this.additionalInformation.text
        }
      }
    }
  }
...
${this.additionalInformation.packageList.map(package => { return `* ${package}` }).join('\n')}
  ...
  async submit () {
    try {
      ...
      if (this.forPackage === 'atom') {
        ...
        [ , owner, repo] = repoUrl.match(/https:\/\/github.com\/([^\/]+)\/([^\/]+)/)
      }
